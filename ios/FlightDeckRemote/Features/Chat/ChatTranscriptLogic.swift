...
extension Wire.TranscriptItem {
    ...
    var itemId: Wire.ItemId {
        ...
        case let .permissionPrompt(itemId, _, _, _, _): itemId
        }
    }
    ...
    var atMs: Int64 {
        ...
        case let .permissionPrompt(_, _, _, _, atMs): atMs
        }
    }

    /// The pending permission-prompt id, if this item is a prompt.
    var permissionPromptId: Wire.PromptId? {
        if case let .permissionPrompt(_, promptId, _, _, _) = self { return promptId }
        return nil
    }
}
